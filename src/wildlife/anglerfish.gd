...
func _physics_process(delta):
	...
	if target != null:
		...
		speed = move_toward(speed, 700, delta * acceleration)
		turn_speed = move_toward(turn_speed, 2, delta * 2)
		
		light.color.g = move_toward(light.color.g, 0, delta)
		light.color.b = move_toward(light.color.g, 0, delta)
	else:
		speed = move_toward(speed, 25, delta * deceleration)
		turn_speed = move_toward(turn_speed, 0.1, delta * 2)
	...
	if target:
		if vector_to_target.length() >= chase_range:
			stop_chase() #distance check				
		elif target.global_position.x < top_left_bound.x or target.global_position.y < top_left_bound.y or \
